#!/bin/zsh
# MyPM을 데스크톱 앱 창(주소창 없는 단독 창)으로 연다.
...
if ! curl -s -o /dev/null --max-time 2 http://localhost:8000/health; then
  echo "백엔드 시작 중..."
  (cd "$ROOT/apps/api" && nohup .venv/bin/python -m uvicorn app.main:app --port 8000 >/tmp/mypm-api.log 2>&1 &)
fi

if ! curl -s -o /dev/null --max-time 2 http://localhost:3000; then
  echo "프론트 시작 중..."
  (cd "$ROOT/apps/web" && nohup npm run dev >/tmp/mypm-web.log 2>&1 &)
fi
