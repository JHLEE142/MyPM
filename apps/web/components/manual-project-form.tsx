...
export function ManualProjectForm() {
  ...
  const stepValid = useMemo(() => {
    ...
    if (step === 2) return workDays.length > 0 && capacity > 0 && capacity <= 24 && buffer >= 0 && buffer <= 90;
    return true;
  ...
  function next() {
    setError("");
    if (!stepValid) {
      setError(step === 1 ? "프로젝트명과 올바른 기간을 입력해 주세요." : "작업 요일과 가용시간을 확인해 주세요.");
      return;
    }
      ...
      <section className="panel p-5 sm:p-8">
        ...
        {step === 2 && (
          <div className="space-y-7">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <label><span className="label">하루 가용시간 *</span><div className="relative"><input className="field pr-14" type="number" min="0.5" max="24" step="0.5" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} /><span className="absolute right-4 top-3 text-sm text-[#71807b]">시간</span></div></label>
              <label><span className="label">버퍼 비율</span><div className="relative"><input className="field pr-12" type="number" min="0" max="90" value={buffer} onChange={(event) => setBuffer(Number(event.target.value))} /><span className="absolute right-4 top-3 text-sm text-[#71807b]">%</span></div><span className="mt-2 block text-xs text-[#71807b]">하루 {capacity}시간 중 {(capacity * (1 - buffer / 100)).toFixed(1)}시간을 기본 배정합니다.</span></label>
            </div>
            <div><span className="label">제외 날짜</span><div className="flex gap-2"><input className="field max-w-xs" type="date" value={excludedInput} onChange={(event) => setExcludedInput(event.target.value)} /><button type="button" className="btn btn-secondary" onClick={addExcluded}>추가</button></div>{excludedDates.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{excludedDates.map((value) => <button type="button" key={value} className="badge badge-neutral" onClick={() => setExcludedDates((current) => current.filter((item) => item !== value))}>{value} ×</button>)}</div>}</div>
          </div>
        )}
        {step === 3 && (
          <div className="space-y-7">
            ...
            <label className="block cursor-pointer rounded-2xl border-2 border-dashed border-[#c9d8d2] bg-[#f7faf8] p-7 text-center hover:border-[#5f9c89]"><input type="file" className="sr-only" accept=".pdf,.docx,.xlsx,.txt,.md,.hwpx" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><span className="block text-2xl" aria-hidden>⇧</span><span className="mt-2 block font-bold">{file ? file.name : "파일을 선택하세요"}</span><span className="mt-1 block text-xs text-[#71807b]">PDF, DOCX, XLSX, TXT, MD, HWPX · 최대 20MB</span></label>
            ...
            <label><span className="label">자료 이름</span><input className="field" value={sourceName} onChange={(event) => setSourceName(event.target.value)} /></label>
            <label><span className="label">회의 메모, 인터뷰 내용, 요구사항</span><textarea className="field min-h-44" value={sourceText} onChange={(event) => setSourceText(event.target.value)} placeholder="텍스트를 붙여넣으세요." /></label>
          </div>
        )}
